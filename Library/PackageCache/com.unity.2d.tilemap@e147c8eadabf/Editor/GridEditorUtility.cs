...
namespace UnityEditor.Tilemaps
{
    internal static class GridEditorUtility
    {
        ...
        {
            Ray ray;
            // Try to get the current camera; fall back to last active SceneView camera
            ...
            if (cam != null && cam.orthographic)
            {
                ...
                // Compute a safe distance along camera forward to avoid out-of-frustum warnings
                ...
                Vector3 cameraWorldPoint = cam.ScreenToWorldPoint(new Vector3(screen.x, screen.y, distance));
                ray = new Ray(cameraWorldPoint, cam.transform.forward);
            }
            else
            {
                // Perspective or no camera available - use GUI point to world ray
            ...
            {
                // If the ray doesn't intersect the plane (parallel), project the transform position
                // onto the ray as a sensible fallback distance.
                Vector3 toPlane = transform.position - ray.origin;
                result = Vector3.Dot(toPlane, ray.direction);
            }
        ...
        public static void DrawGridGizmo(GridLayout gridLayout, Transform transform, Color color, ref Mesh gridMesh, ref Material gridMaterial, bool force = false)
        {
            // TODO: Hook this up with DrawGrid
            if (Event.current.type != EventType.Repaint && !force)
                return;
